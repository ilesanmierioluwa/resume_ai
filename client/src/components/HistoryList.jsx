 ...
 */
export default function HistoryList({ analyses, loading, error, onView, onDismissError }) {
  return (
    <section>
      ...
      <div className="mt-6">
        ...
        {!loading && analyses.length === 0 ? (
          <div className="rounded-xl bg-white p-8 text-center text-slate-600 shadow-md">
            No resume analyses have been saved yet.
          </div>
        ) : null}
        <div className="space-y-4">
          {analyses.map((analysis) => (
            <article key={analysis._id} className="flex flex-col gap-4 rounded-xl bg-white p-5 shadow-md sm:flex-row sm:items-center sm:justify-between">
              <div>
              ...
              <button
                ...
                className="rounded-lg bg-blue-600 px-5 py-2.5 font-semibold text-white shadow-md hover:bg-blue-700"
              >
                View
              </button>
            </article>
